...
for(i in 1:5) {
  chrGenesPlus <- genesPlus[genesPlus$chr == chrs[i],]
  # sense gene "convergent region" = TTS +/- 2000 bp
  chrGenesPlusExtIR <- IRanges(start = pmax(1, chrGenesPlus$end-2000), end = pmin(chrGenesPlus$end+2000, chrLens[i]))
  ...
  chrGenesMinus <- genesMinus[genesMinus$chr == chrs[i],]
  # antisense gene "convergent region" = TTS +/- 2000 bp
  chrGenesMinusExtIR <- IRanges(start = pmax(1, chrGenesMinus$start-2000), end = pmin(chrGenesMinus$start+2000, chrLens[i]))
...
save(allConvergentGR, file = paste0(outDir, "allConvergentGR_2kb_RNAseq.RData"))
