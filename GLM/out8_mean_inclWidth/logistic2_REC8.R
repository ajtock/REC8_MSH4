...
outDir <- "/projects/ajt200/REC8_MSH4/GLM/out8_mean_inclWidth/"
...
glmCO <- glm(co~band+meanREC8_ChIP*(gene+promoter+terminator+RC_Helitron+DNA_Pogo+DNA_Tc1+DNA_Mariner+DNA_MuDR+DNA_Harbinger+DNA_HAT+LTR_Copia+DNA_En.Spm+SINE+LINE_L1+LTR_Gypsy+width), family = binomial(link = "logit"), data = dat, control = list(maxit = 50))
predict1 <- predict(glmCO, type = "response")
glmSum <- summary(glmCO)
coeff <- glmSum$coefficients
save(glmSum, file = paste0(outDir, "GLM_meanREC8_ChIP_summary.RData"))
write.csv(coeff, file = paste0(outDir, "GLM_meanREC8_ChIP_coeffs.csv"))
save(predict1, file = paste0(outDir, "GLM_meanREC8_ChIP_predict.RData"))

glmCO <- glm(co~band+meannuc*(gene+promoter+terminator+RC_Helitron+DNA_Pogo+DNA_Tc1+DNA_Mariner+DNA_MuDR+DNA_Harbinger+DNA_HAT+LTR_Copia+DNA_En.Spm+SINE+LINE_L1+LTR_Gypsy+width), family = binomial(link = "logit"), data = dat, control = list(maxit = 50))
predict2 <- predict(glmCO, type = "response")
glmSum <- summary(glmCO)
coeff <- glmSum$coefficients
save(glmSum, file = paste0(outDir, "GLM_meannuc_summary.RData"))
write.csv(coeff, file = paste0(outDir, "GLM_meannuc_coeffs.csv"))
save(predict2, file = paste0(outDir, "GLM_meannuc_predict.RData"))

glmCO <- glm(co~band+meanspo*(gene+promoter+terminator+RC_Helitron+DNA_Pogo+DNA_Tc1+DNA_Mariner+DNA_MuDR+DNA_Harbinger+DNA_HAT+LTR_Copia+DNA_En.Spm+SINE+LINE_L1+LTR_Gypsy+width), family = binomial(link = "logit"), data = dat, control = list(maxit = 50))
predict3 <- predict(glmCO, type = "response")
glmSum <- summary(glmCO)
coeff <- glmSum$coefficients
save(glmSum, file = paste0(outDir, "GLM_meanSPO11-oligos_summary.RData"))
write.csv(coeff, file = paste0(outDir, "GLM_meanSPO11-oligos_coeffs.csv"))
save(predict3, file = paste0(outDir, "GLM_meanSPO11-oligos_predict.RData"))

glmCO <- glm(co~band+meanMSH4_ChIP*(gene+promoter+terminator+RC_Helitron+DNA_Pogo+DNA_Tc1+DNA_Mariner+DNA_MuDR+DNA_Harbinger+DNA_HAT+LTR_Copia+DNA_En.Spm+SINE+LINE_L1+LTR_Gypsy+width), family = binomial(link = "logit"), data = dat, control = list(maxit = 50))
predict4 <- predict(glmCO, type = "response")
glmSum <- summary(glmCO)
coeff <- glmSum$coefficients
save(glmSum, file = paste0(outDir, "GLM_meanMSH4_ChIP_summary.RData"))
write.csv(coeff, file = paste0(outDir, "GLM_meanMSH4_ChIP_coeffs.csv"))
save(predict4, file = paste0(outDir, "GLM_meanMSH4_ChIP_predict.RData"))

glmCO <- glm(co~band+(meanREC8_ChIP*meanMSH4_ChIP*meannuc*meanspo)*(gene+promoter+terminator+RC_Helitron+DNA_Pogo+DNA_Tc1+DNA_Mariner+DNA_MuDR+DNA_Harbinger+DNA_HAT+LTR_Copia+DNA_En.Spm+SINE+LINE_L1+LTR_Gypsy+width), family = binomial(link = "logit"), data = dat, control = list(maxit = 50))
predict5 <- predict(glmCO, type = "response")
glmSum <- summary(glmCO)
coeff <- glmSum$coefficients
save(glmSum, file = paste0(outDir, "GLM_mean_REC8_MSH4_nuc_SPO11_summary.RData"))
write.csv(coeff, file = paste0(outDir, "GLM_mean_REC8_MSH4_nuc_SPO11_coeffs.csv"))
save(predict5, file = paste0(outDir, "GLM_mean_REC8_MSH4_nuc_SPO11_predict.RData"))
...
pdf(file = paste0(outDir, "meanGLM_REC8_ChIP_hexiles_CO_boxplot.pdf"))
boxplot(lapply(ssID, function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict1[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean REC8 ChIP*(annotation+width)", xlab = "REC8 ChIP hexiles", ylab = "Number of crossovers per Mb", names = c("1", "2", "3", "4", "5", "6"))
...
pdf(file = paste0(outDir, "meanGLM_REC8_ChIP_hexiles_CO_boxplot_lessnoise?.pdf"))
boxplot(lapply(ssID, function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict1[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean REC8 ChIP*(annotation+width)", xlab = "REC8 ChIP hexiles", ylab = "Number of crossovers per Mb", names = c("1", "2", "3", "4", "5", "6"))
tco <- sapply(ssID, function(x) sum(dat$co[x])/sum(dat$width[x])*1e6)
points(x = 1:length(ssID), y = tco, col = "red", pch = 19)
legend("top", legend = c("Observed", "Predicted"), text.col = c("red", "black"), bty = "n")
...
pdf(file = paste0(outDir, "meanGLM_nuc_hexiles_CO_boxplot.pdf"))
tco <- sapply(ssID, function(x) sum(dat$co[x])/sum(dat$width[x])*1e6)
boxplot(lapply(ssID, function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict2[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean nucleosomes*(annotation+width)", xlab = "Nucleosomes hexiles", ylab = "Number of crossovers per Mb", names = c("1", "2", "3", "4", "5", "6"))
points(x = 1:length(ssID), y = tco, col = "red", pch = 19)
legend("top", legend = c("Observed", "Predicted"), text.col = c("red", "black"), bty = "n")
dev.off()

ssID <- split(1:nrow(dat), cut(dat$meannuc, breaks = sort(dat$meannuc)[round(0:7*(nrow(dat)/7))]))
pdf(file = paste0(outDir, "meanGLM_nuc_hexiles_CO_boxplot_lessnoise?.pdf"))
tco <- sapply(ssID, function(x) sum(dat$co[x])/sum(dat$width[x])*1e6)
boxplot(lapply(ssID, function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict2[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean nucleosomes*(annotation+width)", xlab = "Nucleosomes hexiles", ylab = "Number of crossovers per Mb", names = c("1", "2", "3", "4", "5", "6"), ylim = c(min(tco), max(tco)))
points(x = 1:length(ssID), y = tco, col = "red", pch = 19)
legend("top", legend = c("Observed", "Predicted"), text.col = c("red", "black"), bty = "n")
dev.off()
...
pdf(file = paste0(outDir, "meanGLM_SPO11-1-oligos_hexiles_CO_boxplot.pdf"))
tco <- sapply(ssID, function(x) sum(dat$co[x])/sum(dat$width[x])*1e6)
boxplot(lapply(ssID, function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict3[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean SPO11-1-oligos*(annotation+width)", xlab = "SPO11-1-oligos hexiles", ylab = "Number of crossovers per Mb", names = c("1", "2", "3", "4", "5", "6"), ylim = c(min(tco), 53))
points(x = 1:length(ssID), y = tco, col = "red", pch = 19)
legend("top", legend = c("Observed", "Predicted"), text.col = c("red", "black"), bty = "n")
dev.off()

ssID <- split(1:nrow(dat), cut(dat$meanspo, breaks = sort(dat$meanspo)[round(0:7*(nrow(dat)/7))]))
pdf(file = paste0(outDir, "meanGLM_SPO11-1-oligos_hexiles_CO_boxplot_lessnoise?.pdf"))
tco <- sapply(ssID, function(x) sum(dat$co[x])/sum(dat$width[x])*1e6)
boxplot(lapply(ssID, function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict3[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean SPO11-1-oligos*(annotation+width)", xlab = "SPO11-1-oligos hexiles", ylab = "Number of crossovers per Mb", names = c("1", "2", "3", "4", "5", "6"))
points(x = 1:length(ssID), y = tco, col = "red", pch = 19)
legend("top", legend = c("Observed", "Predicted"), text.col = c("red", "black"), bty = "n")
dev.off()
...
pdf(file = paste0(outDir, "meanGLM_MSH4_ChIP_hexiles_CO_boxplot.pdf"))
boxplot(lapply(ssID, function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict4[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean MSH4 ChIP*(annotation+width)", xlab = "MSH4 ChIP hexiles", ylab = "Number of crossovers per Mb", names = c("1", "2", "3", "4", "5", "6"))
tco <- sapply(ssID, function(x) sum(dat$co[x])/sum(dat$width[x])*1e6)
points(x = 1:length(ssID), y = tco, col = "red", pch = 19)
legend("top", legend = c("Observed", "Predicted"), text.col = c("red", "black"), bty = "n")
dev.off()
...
pdf(file = paste0(outDir, "meanGLM_MSH4_ChIP_hexiles_CO_boxplot_lessnoise?.pdf"))
boxplot(lapply(ssID, function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict4[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean MSH4 ChIP*(annotation+width)", xlab = "MSH4 ChIP hexiles", ylab = "Number of crossovers per Mb", names = c("1", "2", "3", "4", "5", "6"))
tco <- sapply(ssID, function(x) sum(dat$co[x])/sum(dat$width[x])*1e6)
points(x = 1:length(ssID), y = tco, col = "red", pch = 19)
legend("top", legend = c("Observed", "Predicted"), text.col = c("red", "black"), bty = "n")
...
pdf(file = paste0(outDir, "meanGLM_REC8_ChIP_chrbands_CO_boxplot.pdf"), height = 6, width = 9)
boxplot(lapply(split(1:nrow(dat), dat$band), function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict1[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+mean REC8 ChIP*(annotation+width)", xlab = "2-Mb chromosome bands", ylab = "Number of crossovers per Mb")
...
pdf(file = paste0(outDir, "meanGLM_REC8_MSH4_nuc_SPO11_chrbands_CO_boxplot.pdf"), height = 6, width = 9)
boxplot(lapply(split(1:nrow(dat), dat$band), function(x) sapply(1:100, function(ii) sum(rbinom(length(x), 1, prob = predict5[x]))/sum(dat$width[x])*1e6)), main = "GLM: CO~band+(mean REC8 ChIP*MSH4 ChIP*nucleosomes*SPO11-1-oligos)*(annotation+width)", xlab = "2-Mb chromosome bands", ylab = "Number of crossovers per Mb")
ssb <- split(1:nrow(dat), dat$band)
points(x = unique(dat$band), y = sapply(ssb, function(x) sum(dat$co[x])/sum(dat$width[x])*1e6), col = "red", pch = 19)
abline(v = c(8, 17, 31, 37, 50), lty = 2, lwd = 2.5)
legend("topleft", legend = c("Observed", "Predicted"), text.col = c("red", "black"), bty = "n")
